...
export default async function AdminDraftNewDogPage({
  ...
  if (resolvedSearchParams?.role === "shelter")
    gateParams.set("role", "shelter");
  ...
  const scopedShelterIds = adminContext.isGlobalAdmin
    ? null
    : adminContext.shelterIds;
  const [{ data: shelters }, { data: personalityTraitRows }] =
    await Promise.all([
  ...
  const selectedShelterId =
    ...
    shelterOptions.some(
      (shelter) => shelter.id === resolvedSearchParams.shelter,
    )
      ? resolvedSearchParams.shelter
      : (shelterOptions[0]?.id ?? "");
  ...
  if (resolvedSearchParams?.role === "shelter")
    listingsParams.set("role", "shelter");
    ...
    return (
      ...
      >
        <section className="rounded-[28px] border border-[#d6c8ad] bg-white/90 p-8 shadow-[0_16px_50px_rgba(101,88,79,0.08)]">
          <h2 className="text-2xl font-semibold text-[#65584f]">
            Add a shelter before creating dog listings.
          </h2>
          <p className="mt-3 max-w-xl text-sm leading-6 text-[#65584f]/75">
            Each dog listing belongs to a shelter, so the draft create flow
            needs at least one shelter record first.
          </p>
          <Link
